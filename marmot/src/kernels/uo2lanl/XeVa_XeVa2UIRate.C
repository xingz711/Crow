...
InputParameters validParams<XeVa_XeVa2UIRate>()
{
  ...
  params.addParam<std::string>("MXeVa2_name", "MXeVa2",  "XeVa2 mobility");
...
XeVa_XeVa2UIRate::XeVa_XeVa2UIRate(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
XeVa_XeVa2UIRate::computeQpResidual()
{
  ...
  Real DF = (_dgHXeVa2UI[_qp] + _kT[_qp] * MathUtils::poly4Log(_c5[_qp],tol,0) - _kT[_qp] * MathUtils::poly4Log(1 - _c5[_qp],tol,0)
             ...
             - (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,0) - 2 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,0)));
  ...
  Real  R1 = -_test[_i][_qp] * (_c6[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa2[_qp]/S/S) * _Zg[_qp] * DF);

  Real  R2 = -_test[_i][_qp] * (-_c6[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa2[_qp]/S/S) * _Zg[_qp] * DF);

  Real  R21 = -_grad_test[_i][_qp] * (_c6[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa2[_qp]/S/S) * _Zg[_qp] * DF2);

  Real  R22 = -_grad_test[_i][_qp] * (-_c6[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa2[_qp]/S/S) * _Zg[_qp] * DF2);

  if (_c6[_qp] >= 0 && _c5[_qp] >= 0)
    return R1 + R21;
  else if (_c6[_qp] < 0.0 && _c5[_qp] < 0.0)
    return R1 + R21;
  else if (_c6[_qp] < 0 || _c5[_qp] < 0)
    return R2 + R22;
  else
    return R2 + R22;
}
...
Real
XeVa_XeVa2UIRate::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
  const Real S = 0.385;
  ...
  Real R1 = -_test[_i][_qp] * (_c6[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa2[_qp]/S/S) * _Zg[_qp]
  ...
  Real R2= _test[_i][_qp] * (_c6[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa2[_qp]/S/S) * _Zg[_qp]
                             * (-_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]
                                - 3 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,1) * (-3 * _phi[_j][_qp])
                                - (-2 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,1) * (-2 * _phi[_j][_qp]))));

  Real R21 = -_grad_test[_i][_qp] * (_c6[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa2[_qp]/S/S) * _Zg[_qp]
                                     * (-_kappa_cgv[_qp] * (_grad_phi[_j][_qp])
                                        - 3 * _kappa_cu[_qp] * (-2 * _grad_phi[_j][_qp])
                                        - (-2 * _kappa_cu[_qp] * (-2 * _grad_phi[_j][_qp]))));

  Real R22 = _grad_test[_i][_qp] * (_c6[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa2[_qp]/S/S) * _Zg[_qp]
                                    * (-_kappa_cgv[_qp] * (_grad_phi[_j][_qp])
                                       - 3 * _kappa_cu[_qp] * (-2 * _grad_phi[_j][_qp])
                                       - (-2 * _kappa_cu[_qp] * (-2 * _grad_phi[_j][_qp]))));
