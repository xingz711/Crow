...
template<>
InputParameters validParams<Va2_XeVa2MRate>()
{
...
Va2_XeVa2MRate::Va2_XeVa2MRate(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
Va2_XeVa2MRate::computeQpResidual()
{

  //Reaction: Xe+VaVa <-> XeVaVa, corrected see comments
  ...
  // Gradient terms
  RealGradient DF2 = (_kappa_cgvv[_qp] * _grad_c6[_qp] - _kappa_cg[_qp] * _grad_c1[_qp] - _kappa_cvv[_qp] * _grad_u[_qp]);

  //  RealGradient DF2 = (_kappa_cgvv[_qp] * _grad_c6[_qp] - _kappa_cg[_qp] * _grad_u[_qp] - _kappa_cvv[_qp] * _grad_c4[_qp]);
...
Real
Va2_XeVa2MRate::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
  ...
  R1 = -_test[_i][_qp] * (_phi[_j][_qp] * _c1[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * DF)
       + _test[_i][_qp] * (_u[_qp] * _c1[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                           * (-_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R2 = -_test[_i][_qp] * (-_phi[_j][_qp] * _c1[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * DF)
       + _test[_i][_qp] * (_u[_qp] * _c1[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                           * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R21 = -_grad_test[_i][_qp] * (_phi[_j][_qp] * _c1[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * DF2)
        + _grad_test[_i][_qp] * (_u[_qp] * _c1[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                                 * (-_kappa_cg[_qp] * _grad_phi[_j][_qp]));

  R22 = -_grad_test[_i][_qp] * (-_phi[_j][_qp] * _c1[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * DF2)
        + _grad_test[_i][_qp] * (_u[_qp] * _c1[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                                 * (+_kappa_cg[_qp] * _grad_phi[_j][_qp]));
