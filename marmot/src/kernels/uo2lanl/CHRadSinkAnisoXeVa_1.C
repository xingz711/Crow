...
CHRadSinkAnisoXeVa_1::CHRadSinkAnisoXeVa_1(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
CHRadSinkAnisoXeVa_1::computeQpResidual()
{
  ...
  for (unsigned int i = 0; i < _n; ++i)
    for (unsigned int j = i + 1; j < _n; ++j)
    {
      ...
      Real inside_g_i = 100.0 * ytot + std::exp(-_vb_i[_qp][ind] / _vm_i[_qp][ind]);
  ...
  return _grad_test[_i][_qp] * _M[_qp] * _u[_qp] * 2.0 * outval1 * 2.0;
}

Real
CHRadSinkAnisoXeVa_1::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];

  Real ytot = 2.0 * _u[_qp] + _c1[_qp];
  RealGradient grad_ytot = 2.0 * _grad_u[_qp] + _grad_c1[_qp];
  ...
  for (unsigned int i = 0; i < _n; ++i)
    for (unsigned int j = i + 1; j < _n; ++j)
    {
      ...
      outval1 += -grad_g_i * f_i + (_vC_i[_qp][ind] - g_i) * grad_f_i;
      doutval1+=(-_vm_i[_qp][ind] * 100 * 2 * _phi[_j][_qp]
                 * MathUtils::poly4Log((100 * ytot + std::exp(-_vb_i[_qp][ind] / _vm_i[_qp][ind])),tol,1) * grad_f_i
                 - ((_vm_i[_qp][ind] * 100 * 2 * _grad_phi[_j][_qp] * (100 * ytot + std::exp(-_vb_i[_qp][ind] / _vm_i[_qp][ind]))
                     - 100 * 2 * _phi[_j][_qp] * _vm_i[_qp][ind] * 100 * grad_ytot)
                    * MathUtils::poly4Log(((100*ytot + std::exp(-_vb_i[_qp][ind] / _vm_i[_qp][ind]))),tol,2) * (-1))
                 * f_i);
