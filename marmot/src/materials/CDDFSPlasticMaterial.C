...
CDDFSPlasticMaterial::CDDFSPlasticMaterial(const InputParameters & parameters) :
...
void CDDFSPlasticMaterial::initQpStatefulProperties()
{
  RealTensorValue rot;
  ...
  while (flag==0)
  {
    // I'm assuming that 'unsigned int' is the correct type since
    // it is compared to _nss.  But the whole thing is weird because
    // _flowprops is a std::vector<Real>
    ind=ind+1;
...
void CDDFSPlasticMaterial::computeQpStress()
{
...
void
CDDFSPlasticMaterial::get_euler_rot()
{
...
void
CDDFSPlasticMaterial::get_slip_sys()
{
...
void
CDDFSPlasticMaterial::update_gss_tau(std::vector<Real> & /*slip_incr*/,
                                     std::vector<Real> & tau)
{
  ...
  Real l_inv = 0.0;
  for (unsigned int i = 0; i < _nss; ++i)
    l_inv = l_inv + (_rhoM[_qp][i] + _rhoI[_qp][i]);
  l_inv = std::sqrt(std::abs(l_inv));
  ...
  for (unsigned int i = 0; i < _nss; ++i)
  {
    ...
    tau_sr = _a * _b * _mu * std::sqrt(std::abs(tau_sr));

    // TODO this will need updating when including irradiation
    tau_ir = _beta * _b * _mu * std::sqrt(_rhoDL_0*_dl_0);
  ...
  for (unsigned int i = 0; i < _nss; ++i)
  {
    ...
    Real signT = 1.0;
    if (tau[i] != 0.0)
      signT = tau[i]/std::abs(tau[i]);
    // EIB - this is tauT from CDD
    // EIB - keep taking this requirement in and out
    // Real vtol = 1.0e-6;
    Real factor = 1.0;
    if (i>11)
      factor = _ani;
    ...
    if (std::abs(tau[i]/(_tau0*factor)) >= 1.0 )
      v_g = _v0 * std::pow(std::abs(tau[i])/_gss[_qp][i],(1.0/_xm[i])) * signT;

    Real RoV =  _rhoM[_qp][i] * v_g;

    // calc each dislocation density term
    // Term 1: alpha1 * _rhoM * |vg|/|l|
    // term1 = _alpha_1 * _rhoM[_qp][i] * std::abs(v_g) * l_inv;
    term1 = _alpha_1 * std::abs(RoV) * l_inv;
    ...
    // term3 = _alpha_3 * _rhoM[_qp][i] * std::abs(v_g) * l_inv;
    term3 = _alpha_3 * std::abs(RoV) * l_inv;
    ...
    //  term4 = _alpha_4 * std::sqrt(std::abs(tau[i])/(_gss[_qp][i]-_tau0)) * _rhoI[_qp][i] * v_g * l_inv;
    term4 = _alpha_4 * std::sqrt(std::abs(tau[i])/(_gss[_qp][i])) * _rhoI[_qp][i] * std::abs(v_g) * l_inv;
    // }

    // TODO the 1.5 is really r - needs to be addded later

    // Term 5: alpha5 * Sum_b ProbMtx(ab)*_rhoM(b)*|vg(a)|/|l|
    term5 = 0.0;
    for (unsigned int j=0; j<_nss; j++)
      term5 = term5 + _alpha_5 * CRM(i,j) * _rhoM[_qp][j] * std::abs(v_g) * l_inv;
    ...
    // _rhoM[_qp][i] = _rhoM_old[_qp][i] + rhoM_rate*_dt;
    // _rhoI[_qp][i] = _rhoI_old[_qp][i] + rhoI_rate*_dt;
    _rhoM[_qp][i] = _rhoM[_qp][i] + rhoM_rate*_dt;
    _rhoI[_qp][i] = _rhoI[_qp][i] + rhoI_rate*_dt;

    /*
    _rhoM[_qp][i] = (_rhoM_old[_qp][i] + term5*_dt +
                    _alpha_4*std::sqrt(std::abs(tau[i])/(_gss[_qp][i]))*_rhoI[_qp][i]*std::abs(v_g)*l_inv*_dt) /
              (1.0 -( _alpha_1*std::abs(v_g)*l_inv - 2.0*_alpha_2*(_Rc*_b)*std::abs(RoV) -
                    _alpha_3 *std::abs(v_g)*l_inv - _alpha_6*(_Rc*_b)*_rhoI[_qp][i]*std::abs(v_g))*_dt);
    _rhoI[_qp][i] = (_rhoI_old[_qp][i] + _alpha_3*std::abs(RoV)*l_inv*_dt) /
              (1.0 - (-_alpha_4*sqrt(std::abs(tau[i])/(_gss[_qp][i]))*std::abs(v_g)*l_inv -
                    _alpha_6*(_Rc*_b)*std::abs(RoV))*_dt);
    */
...
void
CDDFSPlasticMaterial::calc_resid_jacob(RankTwoTensor & pk2,
                                       RankTwoTensor & sig,
...
void
CDDFSPlasticMaterial::get_slip_incr(std::vector<Real> & tau,
                                    std::vector<Real> & slip_incr,
                                    std::vector<Real> & dslipdtau)
{
  for (unsigned int i=0; i<_nss; i++)
    slip_incr[i] = _rhoM[_qp][i] * _b * _v0 * std::pow(std::abs(tau[i]) / _gss[_qp][i], (1.0/_xm[0])) * MathUtils::sign(tau[i]) * _dt;

  for (unsigned int i=0; i<_nss; i++)
    dslipdtau[i] = _rhoM[_qp][i] * _b * _v0 / _xm[i] * std::pow(std::abs(tau[i]) / _gss[_qp][i], (1.0/_xm[0]-1.0)) / _gss[_qp][i] * _dt;
}

RankTwoTensor CDDFSPlasticMaterial::getmatrot(RankTwoTensor &a)
{
  RankTwoTensor rot;
...
void
CDDFSPlasticMaterial::get_euler_ang()
{
...
void CDDFSPlasticMaterial::computeQpElasticityTensor()
{
