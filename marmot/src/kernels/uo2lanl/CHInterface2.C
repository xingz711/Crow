...
CHInterface2::CHInterface2(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
CHInterface2::computeQpResidual()
{
  //Calculates the gradient energy term specifically for the Xe, Va, XeVa and VaVa system

  RealTensor second_c = -_second_c1[_qp]-_second_c2[_qp]-2*_second_c3[_qp]-2*_second_c4[_qp];
...
Real
CHInterface2::computeQpJacobian()
{
  RealTensor second_c = -_second_c1[_qp]-_second_c2[_qp]-2*_second_c3[_qp]-2*_second_c4[_qp];
