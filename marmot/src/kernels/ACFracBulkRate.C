...
InputParameters validParams<ACFracBulkRate>()
{
  ...
  params.addParam<Real>("gc_tens", 1.0, "Interface energy");
  params.addParam<Real>("gc_comp", 1.0, "Interface energy");
...
ACFracBulkRate::ACFracBulkRate(const InputParameters & parameters):
    KernelValue(parameters),
    _L(getMaterialProperty<Real>("mob_name")),
    _gc_prop_tens(getMaterialProperty<Real>("gc_prop_tens")),
    _gc_prop_comp(getMaterialProperty<Real>("gc_prop_comp")),
    _G0_pos(getMaterialProperty<Real>("G0_pos")),
    _G0_neg(getMaterialProperty<Real>("G0_neg")),
    ...
    _gc_comp(getParam<Real>("gc_comp")),
    _l(getParam<Real>("l")),
...
ACFracBulkRate::computeDFDOP(PFFunctionType type)
{
  switch (type)
  {
    case Residual:
    {
      ...
      Real gc_tens = _gc_tens;
      if (_gc_prop_tens[_qp] > 0.0)
        gc_tens=_gc_prop_tens[_qp];

      Real gc_comp = _gc_comp;
      if (_gc_prop_comp[_qp] > 0.0)
        gc_comp = _gc_prop_comp[_qp];

      /* if (c>1.0) c=1.0;
      if (c<0.0) c=0.0;*/

      Real x =   _betaval[_qp]
               + 2.0*(1.0-c) * (_G0_pos[_qp]/gc_tens+_G0_neg[_qp]/gc_comp)-c/_l;
    ...
    case Jacobian:
    {
      ...
      Real x = _betaval[_qp] + 2.0 * (1.0 - c) * (_G0_pos[_qp] / gc_tens + _G0_neg[_qp] / gc_comp) - c / _l;
      Real signx = x > 0.0 ? 1.0 : -1.0;
      return (signx + 1.0)/2.0 *
                 (2.0*(_G0_pos[_qp]/gc_tens+_G0_neg[_qp]/gc_comp)+1.0/_l)/_visco;
    }
  }
