...
XeVa2_XeVa2MRate::XeVa2_XeVa2MRate(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
XeVa2_XeVa2MRate::computeQpResidual()
{
  ...
  // Gradient terms
  RealGradient DF2 = (_kappa_cgvv[_qp] * _grad_u[_qp] - _kappa_cg[_qp] * _grad_c1[_qp] - _kappa_cvv[_qp] * _grad_c4[_qp]);
...
Real
XeVa2_XeVa2MRate::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
  ...
  R21 = _grad_test[_i][_qp] * (_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]) * (_kappa_cgvv[_qp] * _grad_phi[_j][_qp]);

  R22 = _grad_test[_i][_qp] * (-_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]) * (_kappa_cgvv[_qp] * _grad_phi[_j][_qp]);
