...
template<>
InputParameters validParams<UO2AnisoConsTempXeUVa2Xe>()
{
  ...
  params.addParam<Real>("kappa_cggv",1.0, "The kappa parameter for the cgv concentration");
  ...
  params.addRequiredParam<Real>("D0GGV", "CGGV2 (XeXeVaVa) pre-exponential factor in nm^2/s");
  params.addRequiredParam<Real>("HGGV", "CGGV2 (XeXeVaVa) migration barrier in eV");
  ...
  //c1=cg, c2=cv, c3=cgv, c4=cvv, c5=cui, c6=cgvv, c7=cggvv
...
UO2AnisoConsTempXeUVa2Xe::UO2AnisoConsTempXeUVa2Xe(const InputParameters & parameters) :
    Material(parameters),
    _c1(coupledValue("c1")),
    _c2(coupledValue("c2")),
    _c3(coupledValue("c3")),
    ...
    _kappa_cggv(getParam<Real>("kappa_cggv")),
    ...
    _EBXeXeVa(getParam<Real>("EBXeXeVa")),
    _EBXeVaXeVa(getParam<Real>("EBXeVaXeVa")),
    _EBVaVa(getParam<Real>("EBVaVa")),
    _SBXeVa(getParam<Real>("SBXeVa")),
    _SBXeVa2(getParam<Real>("SBXeVa2")),
    _SBXeVa2B(getParam<Real>("SBXeVa2B")),
    _SBXeXeVa(getParam<Real>("SBXeXeVa")),
    _SBXeVaXeVa(getParam<Real>("SBXeVaXeVa")),
    ...
    _HGGV(getParam<Real>("HGGV")),
    _D0GGV(getParam<Real>("D0GGV")),
    _Z(getParam<Real>("Z")),
    ...
    _EBgXeXeVa(declareProperty<Real>("EBgXeXeVa")),
    _EBgXeVaXeVa(declareProperty<Real>("EBgXeVaXeVa")),
    ...
    _SBgXeXeVa(declareProperty<Real>("SBgXeXeVa")),
    _SBgXeVaXeVa(declareProperty<Real>("SBgXeVaXeVa")),
    ...
    _Dggv(declareProperty<Real>("Dggv")),
    _Mggv(declareProperty<Real>("Mggv")),
    _grad_Mggv(declareProperty<RealGradient>("grad_Mggv")),
    ...
    _kappa_cGGV(declareProperty<Real>("kappa_cGGV")),
    _kappa_cGGVU(declareProperty<Real>("kappa_cGGVU")),
...
void
UO2AnisoConsTempXeUVa2Xe::computeProperties()
{

  //Materils file for Xe, Va, XeVa, VaVa, UI, XeVaVa, XeXeVaVa system
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
  {
    ...
    _Dggv[_qp] = D0GGV * std::exp(-DHGGV/_kT[_qp]); // nm^2/s
    _Mggv[_qp] = _Dggv[_qp]/_kT[_qp]; // nm^2/(s*eV);
    ...
    RealGradient grad_DGGV = -D0B * grad_DHGGV/_kT[_qp] * std::exp(-DHGGV/_kT[_qp]);
    ...
    _grad_Mggv[_qp] = grad_DGGV/_kT[_qp];
    ...
    _EBgXeXeVa[_qp]= _EBXeXeVa; // eV
    _EBgXeVaXeVa[_qp]= _EBXeVaXeVa; // eV
    _EBgVaVa[_qp]= _EBVaVa; // eV

    _SBgXeVa[_qp]= _SBXeVa; // kB
    _SBgXeVa2[_qp]= _SBXeVa2; // kB
    _SBgXeVa2B[_qp]= _SBXeVa2B; // kB
    _SBgXeXeVa[_qp]= _SBXeXeVa; // kB
    _SBgXeVaXeVa[_qp]= _SBXeVaXeVa; // kB
    _SBgVaVa[_qp]= _SBVaVa; // kB


    // Calculate other concentrations based on c1, c2, cgp, cvap;
    // these are the ones that enter the new model
    _cu[_qp]=1-_c1[_qp]-_c2[_qp]-2*_c3[_qp]-2*_c4[_qp]-3*_c6[_qp]-4*_c7[_qp];
    _grad_cu[_qp]=-_grad_c1[_qp]-_grad_c2[_qp]-2*_grad_c3[_qp]-2*_grad_c4[_qp] - 3*_grad_c6[_qp] - 4*_grad_c7[_qp];
    ...
    _kappa_cGGVU[_qp] = -4.0*_kappa_cggv;
