...
CHChemPotential::computeGradDFDCons(PFFunctionType type)
{
  ...
  {
    case Residual:
      return 34*_grad_u[_qp] + 12*16*_u[_qp]*_u[_qp]*_grad_u[_qp] - 12*16*_u[_qp]*_grad_u[_qp]; // return Residual value

    case Jacobian:
      return 34*_grad_phi[_j][_qp] + 24*16*_u[_qp]*_grad_u[_qp]*_phi[_j][_qp] + 12*16*_u[_qp]*_u[_qp]*_grad_phi[_j][_qp] - 12*16*_u[_qp]*_grad_phi[_j][_qp] - 12*16*_grad_u[_qp]*_phi[_j][_qp] ; //return Jacobian value
  }
