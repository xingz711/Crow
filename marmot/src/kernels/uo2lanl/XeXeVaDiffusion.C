...
template<>
InputParameters validParams<XeXeVaDiffusion>()
{
...
XeXeVaDiffusion::XeXeVaDiffusion(const InputParameters & parameters) :
    CHBulk(parameters),
...
XeXeVaDiffusion::computeGradDFDCons(PFFunctionType type)
{

  //XeXeVa or XeXeVaVa diffusion

  switch (type)
  {
    case Residual:
      return _kT[_qp] * (_grad_u[_qp] - 4 * _u[_qp] / (_cu[_qp]) * _grad_cu[_qp]);

    case Jacobian:

      return _kT[_qp] * (
               _grad_phi[_j][_qp]
               - 4.0 * _phi[_j][_qp] / (_cu[_qp]) * _grad_cu[_qp]
               - 4.0 * _u[_qp] / (_cu[_qp]) * (-4.0 * _grad_phi[_j][_qp])
               - 4.0 * _u[_qp] / (_cu[_qp]) / (_cu[_qp]) * (-4.0 * _phi[_j][_qp]) * _grad_cu[_qp]
             );
  }
