...
CHRadSinkAnisoXeVa_2::CHRadSinkAnisoXeVa_2(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
CHRadSinkAnisoXeVa_2::computeQpResidual()
{
  ...
  Real ytot = 2 * _u[_qp] + _c1[_qp] + _c2[_qp] + 2 * _c4[_qp];
  RealGradient grad_ytot = 2 * _grad_u[_qp] + _grad_c1[_qp] + _grad_c2[_qp] + 2 * _grad_c4[_qp];
...
Real
CHRadSinkAnisoXeVa_2::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
      ...
      doutval1 += (-_vVam_i[_qp][ind] * 100 * 2 * _phi[_j][_qp]
                   * MathUtils::poly4Log((100 * ytot + std::exp(-_vVab_i[_qp][ind] / _vVam_i[_qp][ind])),tol,1) * grad_f_i
                   - ((_vVam_i[_qp][ind] * 100 * 2 * _grad_phi[_j][_qp] * (100 * ytot + std::exp(-_vVab_i[_qp][ind] / _vVam_i[_qp][ind]))
                      ...
                      * MathUtils::poly4Log(((100 * ytot + std::exp(-_vVab_i[_qp][ind] / _vVam_i[_qp][ind]))),tol,2) * (-1))
                   * f_i);
