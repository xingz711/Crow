...
InputParameters validParams<BicrystalCircleGrainVoidICAction>()
{
  ...
  params.addParam<bool>("3D_spheres", true, "in 3D, whether the voids are spheres or columns");
...
BicrystalCircleGrainVoidICAction::BicrystalCircleGrainVoidICAction(const InputParameters & params) :
    Action(params),
...
void
BicrystalCircleGrainVoidICAction::act()
{
#ifdef DEBUG
  Moose::err << "Inside the BicrystalCircleGrainVoidICAction Object\n";
#endif
  ...
  for (unsigned int op = 0; op < _op_num; op++)
  {
    ...
    InputParameters poly_params = _factory.getValidParams("CircleGrainVoidIC");
    ...
    poly_params.set<Point>("grain_center") = getParam<Point>("grain_center");
    poly_params.set<Real>("grain_radius") = getParam<Real>("grain_radius");
    ...
    if (op == 0)
      ...
      poly_params.set<unsigned int>("op_index") = 1;

    //Add initial condition
    _problem->addInitialCondition("CircleGrainVoidIC", name() + "_" + Moose::stringify(op), poly_params);
  }
}
