...
Va_Va2UIRate::Va_Va2UIRate(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
Va_Va2UIRate::computeQpResidual()
{

  //Reaction: VaVa + UI <-> Va + Va_I
  ...
  Real DF =  _dgHVaUI[_qp] + _EBVaVa[_qp]
             ...
             + _kT[_qp] * MathUtils::poly4Log(_c4[_qp], tol, 0) - 2.0 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp], tol, 0)
             - (_kT[_qp] * MathUtils::poly4Log(_u[_qp], tol, 0) - 1.0 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp], tol, 0));

  // Gradient terms
  RealGradient DF2 = (_kappa_cui[_qp] * _grad_c5[_qp] - 1 * _kappa_cui[_qp] * (-_grad_c5[_qp]) + _kappa_cvv[_qp] * _grad_c4[_qp]
                      - 2 * _kappa_cu[_qp] * _grad_CU[_qp] - (_kappa_cv[_qp] * _grad_u[_qp] - 1 * _kappa_cu[_qp] * _grad_CU[_qp]));

  Real  R1 = -_test[_i][_qp] * (_c4[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * DF);

  Real  R2 = -_test[_i][_qp] * (-_c4[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * DF);

  Real  R21 = -_grad_test[_i][_qp] * (_c4[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * DF2);

  Real  R22 = -_grad_test[_i][_qp] * (-_c4[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * DF2);

  if (_c4[_qp] >= 0 && _c5[_qp] >= 0)
    return R1 + R21;
  else if (_c4[_qp] < 0.0 && _c5[_qp] < 0.0)
    return R1 + R21;
  else if (_c4[_qp] < 0 || _c5[_qp] < 0)
    return R2 + R22;
  else
    return R2 + R22;
...
Real
Va_Va2UIRate::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
  ...
  RealGradient DF2 = (_kappa_cui[_qp] * _grad_c5[_qp] - 1 * _kappa_cui[_qp] * (-_grad_c5[_qp])
                      + _kappa_cvv[_qp] * _grad_c4[_qp] - 2 * _kappa_cu[_qp] * _grad_CU[_qp]
                      - (_kappa_cv[_qp] * _grad_u[_qp] - 1 * _kappa_cu[_qp] * _grad_CU[_qp]));

  Real R1 = -_test[_i][_qp] * (_c4[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp]
                               * (-2 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,1) * (-_phi[_j][_qp])
                                  - _kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp])
                               + 1 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,0) * (-_phi[_j][_qp]));

  Real R2= _test[_i][_qp] * (_c4[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp]
                             * (-2 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,1) * (-_phi[_j][_qp])
                                - _kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp])
                             + 1 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,0) * (-_phi[_j][_qp]));

  Real R21 = -_grad_test[_i][_qp] * (_c4[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S)
                                     * (-2 * _kappa_cu[_qp] * (-_grad_phi[_j][_qp]) - _kappa_cv[_qp] * _grad_phi[_qp][_qp])
                                     + 1 * _kappa_cu[_qp] * (-_grad_phi[_j][_qp]));

  Real R22 = _grad_test[_i][_qp] * (_c4[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S)
                                    * (-2 * _kappa_cu[_qp] * (-_grad_phi[_j][_qp]) - _kappa_cv[_qp] * _grad_phi[_qp][_qp])
                                    + 1 * _kappa_cu[_qp] * (-_grad_phi[_j][_qp]));
