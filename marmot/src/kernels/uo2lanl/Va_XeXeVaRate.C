...
Va_XeXeVaRate::Va_XeXeVaRate(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
Va_XeXeVaRate::computeQpResidual()
{

  //Reaction: XeVaVa + Xe <-> XeXeVa+Va, not used with XeVaVa + Xe <-> XeXeVaVa
  ...
  RealGradient DF2 = (_kappa_cggv[_qp] * _grad_c7[_qp]
                      ...
                      - _kappa_cg[_qp] * _grad_c1[_qp]
                      - _kappa_cgvv[_qp]*_grad_c6[_qp]);

  R1 = _test[_i][_qp] * (_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp] * DF);

  R2 = _test[_i][_qp] * (-_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp] * DF);

  R21 = _grad_test[_i][_qp] * (_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp] * DF2);

  R22 = _grad_test[_i][_qp] * (-_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp] * DF2);

  // Check sign to avoid negative concentrations
  if (_c1[_qp] >= 0 && _c6[_qp] >= 0)
    return R1 + R21;
  else if (_c1[_qp] < 0.0 && _c6[_qp] < 0.0)
    return R1 + R21;
  else if (_c1[_qp] < 0 || _c6[_qp] < 0)
    return R2 + R22;
  else
    return R2 + R22;
}
...
Real
Va_XeXeVaRate::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
  ...
  R1a = _test[_i][_qp] * (_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp] * DF);

  R21a = _grad_test[_i][_qp] * (_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp] * DF2);

  R1 = _test[_i][_qp] * (_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp]
                         * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R2 = _test[_i][_qp] * (-_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp]
                         * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R21 = _grad_test[_i][_qp] * (_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp])
        * (_kappa_cv[_qp] * _grad_phi[_j][_qp]);

  R22 = _grad_test[_i][_qp] * (-_c1[_qp] * _c6[_qp] * (_MXeXe[_qp]/S/S) * _Zg[_qp])
        * (_kappa_cv[_qp] * _grad_phi[_j][_qp]);
