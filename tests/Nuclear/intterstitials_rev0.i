...
[Kernels]
  active = 'AC_int w_res c_dot e_dot c_res CH_noise ci_res AC_bulk wi_res VacancyAhhihilation ci_dot'
  [./c_dot]
...
[Materials]
  ...
  [./Energy_matrix]
    ...
    block = 0
    function = '(e-1)^2*(Ef*cv + Ef*ci+ kbT*(cv*log(cv)+ci*log(ci)+(1-cv-ci)*log(1-cv-ci))) + B*((cv-1)^2+ci^2)*e^2'
    args = 'cv e ci'
    constant_names = 'Ef   kbT cv0 B'
    constant_expressions = '9.09  0.11 1.13e-4 9.09'
