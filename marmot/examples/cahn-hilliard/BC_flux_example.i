# This example models bubble evolution where there is an inward flux of atoms
# on one side and an outward flux on the other.
...
[Postprocessors]
  ...
  [./n_bubbles]
    ...
    variable = c
    # bubble_volume_file = bubble_volume # Used to output file with bubble volumes
  [../]
[]
...
[Executioner]
  ...
  nl_rel_tol = 1.0e-8 # Relative tolerance for nonlienar solves
  start_time = 0.0
  num_steps = 100
  [./TimeStepper]
    type = IterationAdaptiveDT
    dt = 2.0e1 # Initial time step
    growth_factor = 1.25 # Maximum about by which the time step and increase each step
    optimal_iterations = 8 # Time step will change to maintain this number of nonlinear iterations
    cutback_factor = 0.75
  [../]
  [./Adaptivity]
