...
XeVa2Rate::XeVa2Rate(const InputParameters & parameters) :
    Kernel(parameters),
...
XeVa2Rate::computeQpResidual()
{
  ...
  Real DF = (_EBXeVa[_qp] - _EBVaVa[_qp] - _kT[_qp]*(_SBXeVa[_qp] - _SBVaVa[_qp])
             + _kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,0) + _kT[_qp] * MathUtils::poly4Log(_c2[_qp],tol,0)
             - _kT[_qp] * MathUtils::poly4Log(_c1[_qp],tol,0) - _kT[_qp] * MathUtils::poly4Log(_c4[_qp],tol,0));

  // Gradient terms
  RealGradient DF2 = (_kappa_cgv[_qp] * _grad_u[_qp] + _kappa_cv[_qp] * _grad_c2[_qp]
...
XeVa2Rate::computeQpJacobian()
{
  ...
  R1 = _test[_i][_qp] * (_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                         * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R2 = _test[_i][_qp] * (-_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                         * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R21 = _grad_test[_i][_qp] * (_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp])
        * (_kappa_cgv[_qp] * _grad_phi[_j][_qp]);

  R22 = _grad_test[_i][_qp] * (-_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp])
        * (_kappa_cgv[_qp]*_grad_phi[_j][_qp]);
