...
Xe_XeVa2Rate::Xe_XeVa2Rate(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
Xe_XeVa2Rate::computeQpResidual()
{
  ...
  Real DF = (_EBXeVa[_qp] - _EBVaVa[_qp] - _kT[_qp] * (_SBXeVa[_qp] - _SBVaVa[_qp])
             + _kT[_qp] * poly4Log(_c3[_qp],tol,0) + _kT[_qp]*poly4Log(_c2[_qp],tol,0)
...
Real
Xe_XeVa2Rate::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
  ...
  R1 = -_test[_i][_qp] * (_phi[_j][_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                          * (_EBXeVa[_qp] - _EBVaVa[_qp] - _kT[_qp] * (_SBXeVa[_qp] - _SBVaVa[_qp])
                             + _kT[_qp]*poly4Log(_c3[_qp],tol,0) + _kT[_qp]*poly4Log(_c2[_qp],tol,0)
                             - _kT[_qp]*poly4Log(_u[_qp],tol,0) - _kT[_qp]*poly4Log(_c4[_qp],tol,0)))
       - _test[_i][_qp] * (_u[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * (-_kT[_qp] * poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R2 = -_test[_i][_qp] * (-_phi[_j][_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                          * (_EBXeVa[_qp] - _EBVaVa[_qp] - _kT[_qp] * (_SBXeVa[_qp] - _SBVaVa[_qp])
                             + _kT[_qp]*poly4Log(_c3[_qp],tol,0) + _kT[_qp] * poly4Log(_c2[_qp],tol,0)
                             - _kT[_qp]*poly4Log(_u[_qp],tol,0) - _kT[_qp]*poly4Log(_c4[_qp],tol,0)))
       - _test[_i][_qp] * (-_u[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * (-_kT[_qp] * poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));
