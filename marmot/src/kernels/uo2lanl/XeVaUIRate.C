...
XeVaUIRate::XeVaUIRate(const InputParameters & parameters) :
    Kernel(parameters),
...
XeVaUIRate::computeQpResidual()
{
  ...
  Real DF = (_dgHXeVaUI[_qp] + _kT[_qp] * MathUtils::poly4Log(_c5[_qp],tol,0)
             - _kT[_qp] * MathUtils::poly4Log(1 - _c5[_qp],tol,0) + _kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,0)
             - 2 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,0) - (_kT[_qp] * MathUtils::poly4Log(_c1[_qp],tol,0)
                 - _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,0)));

  // Gradient terms
  RealGradient DF2 = (_kappa_cui[_qp] * _grad_c5[_qp] - 1 * _kappa_cui[_qp] * (-_grad_c5[_qp])
                      + _kappa_cgv[_qp] * _grad_u[_qp] - 2 * _kappa_cu[_qp] * _grad_CU[_qp]
                      - (_kappa_cg[_qp] * _grad_c1[_qp] - _kappa_cu[_qp] * _grad_CU[_qp]));

  Real  R1 = _test[_i][_qp] * (_u[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp] * DF);

  Real  R2 = _test[_i][_qp] * (-_u[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp] * DF);

  Real  R21 = _grad_test[_i][_qp] * (_u[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp] * DF2);

  Real  R22 = _grad_test[_i][_qp] * (-_u[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp] * DF2);
...
XeVaUIRate::computeQpJacobian()
{
  ...
  Real R1 = _test[_i][_qp] * (_phi[_j][_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp]) * DF
            + _test[_i][_qp] * (_u[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp]
  ...
  Real R2= -_test[_i][_qp] * (_phi[_j][_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp]) * DF
           - _test[_i][_qp] * (_u[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp]
                               * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp] - 2 * _kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,1) * (-_phi[_j][_qp])
                                  - (-_kT[_qp] * MathUtils::poly4Log(_CU[_qp],tol,1) * (-_phi[_j][_qp]))));

  Real R21 = _grad_test[_i][_qp] * (_phi[_j][_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp] * DF2)
             + _grad_test[_i][_qp] * (_u[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp]
                                      * (_kappa_cgv[_qp] * (_grad_phi[_j][_qp])
                                         - 2 * _kappa_cu[_qp] * (-2 * _grad_phi[_j][_qp]) - (-_kappa_cu[_qp] * (-2 * _grad_phi[_j][_qp]))));

  Real R22 = -_grad_test[_i][_qp] * (_phi[_j][_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp] * DF2)
             - _grad_test[_i][_qp] * (_u[_qp] * _c5[_qp] * (_MUI[_qp]/S/S + _MXeVa[_qp]/S/S) * _Zg[_qp]
                                      * (_kappa_cgv[_qp] * (_grad_phi[_j][_qp]) - 2 * _kappa_cu[_qp] * (-2 * _grad_phi[_j][_qp])
                                         - (-_kappa_cu[_qp] * (-2 * _grad_phi[_j][_qp]))));
