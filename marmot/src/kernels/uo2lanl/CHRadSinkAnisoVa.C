...
CHRadSinkAnisoVa::CHRadSinkAnisoVa(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
CHRadSinkAnisoVa::computeQpResidual()
{

  //Segregation of Va to grain boundaries, no longer in use or up to date
  ...
  RealGradient grad_ytot = _grad_u[_qp] + _grad_c1[_qp] + 2 * _grad_c3[_qp] + 2 * _grad_c4[_qp];
  ...
  for (unsigned int i = 0; i < _n; ++i) {
    for (unsigned int j = i + 1; j < _n; ++j)
    {
      ...
      Real dgidc = 100.0 * _vVam_i[_qp][ind] * MathUtils::poly4Log(inside_g_i, tol, 1);
...
Real
CHRadSinkAnisoVa::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
  ...
  for (unsigned int i = 0; i < _n; ++i)
    for (unsigned int j = i + 1; j < _n; ++j)
    {
      ...
      doutval1 += ( -_vVam_i[_qp][ind] * 100 * _phi[_j][_qp]
                    * MathUtils::poly4Log((100*ytot + std::exp(-_vVab_i[_qp][ind] / _vVam_i[_qp][ind])),tol,1) * grad_f_i
                    - ((_vVam_i[_qp][ind] * 100 * _grad_phi[_j][_qp] * (100 * ytot + std::exp(-_vVab_i[_qp][ind] / _vVam_i[_qp][ind])) - 100 * _phi[_j][_qp] * _vVam_i[_qp][ind] * 100 * grad_ytot) * MathUtils::poly4Log(((100*ytot + std::exp(-_vVab_i[_qp][ind] / _vVam_i[_qp][ind]))),tol,2) * (-1)
                      ) * f_i);
