...
// This material computes the amount the vacancy and interstitial concentrations will increase at each quadrature point due to neutron irradiation.
...
InputParameters validParams<NeutronSrc>()
{
  ...
  params.addParam<bool>("periodic",false, "Are you using periodic boundary conditions?");

  params.addParam<int>("num_neutrons",1, "Number of neutron hits per time step, if 0 then it is random");
  params.addParam<Real>("expected_num",0.0,"For a random number of neutrons, the expected value per unit area/volume per unit time");
...
NeutronSrc::NeutronSrc(const InputParameters & parameters) :
    Material(parameters),
    ...
    _input_num_neutrons(getParam<int>("num_neutrons")),
    _num_neutrons(0),
...
void
NeutronSrc::timestepSetup()
{
  MooseRandom::seed(_t_step+123); // sketchy (should use RandomInterface)
  ...
  if (_input_num_neutrons == 0 && scaled_expected_num > 0)
  {
    ...
    // _num_neutrons = (Real)MooseRandom::rand() / (Real)RAND_MAX*_range(0)*_range(1)*2.0*_expected_num*_dt;

    // The 0.5 is for rounding
    _num_neutrons = (std_normal*std_dev + meanvalue) + 0.5;

    if (_t_step==1 && _num_neutrons<1)
      _num_neutrons = 1;
  ...
  else if (_input_num_neutrons > 0)
    _num_neutrons = _input_num_neutrons;

  // _console << "scaled expected number = " << scaled_expected_num << ", num neutrons = " << _num_neutrons << std::endl;

  // Resize vector of neutron impact positions
  _NImpPos.resize(_num_neutrons);

  // Randomly determine the neutron impact positions
  for (int i=0; i<_num_neutrons; i++)
  {
...
NeutronSrc::computeProperties()
{
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
  {
    ...
    if (_t_step > 0 && _NImpPos.size() > 0)
    {
      ...
      for (int ht=0; ht<_num_neutrons; ht++)
      {
