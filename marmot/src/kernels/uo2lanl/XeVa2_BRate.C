...
XeVa2_BRate::XeVa2_BRate(const InputParameters & parameters) :
    Kernel(parameters),
...
XeVa2_BRate::computeQpResidual()
{
  ...
  // Concentration terms
  Real DF = (_EBXeVa2[_qp] - _kT[_qp] * _SBXeVa2[_qp] + _kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,0)
             - _kT[_qp] * MathUtils::poly4Log(_c1[_qp],tol,0) - _kT[_qp] * MathUtils::poly4Log(_c4[_qp],tol,0));

  RealGradient DF2 = (_kappa_cgv[_qp] * _grad_u[_qp] - _kappa_cg[_qp] * _grad_c1[_qp] - _kappa_cv[_qp] * _grad_c4[_qp]);

  Real R1 = _test[_i][_qp] * (_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * DF);

  Real R2 = _test[_i][_qp] * (-_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * DF);

  Real R21 = _grad_test[_i][_qp] * (_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * DF2);

  Real R22 = _grad_test[_i][_qp] * (-_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * DF2);
...
XeVa2_BRate::computeQpJacobian()
{
  ...
  Real R1 = _test[_i][_qp] * (_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                              * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  Real R2 = _test[_i][_qp] * (-_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp]
                              * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  Real R21 = _grad_test[_i][_qp] * (_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * _kappa_cgv[_qp] * (_grad_phi[_j][_qp]));

  Real R22 = _grad_test[_i][_qp] * (-_c1[_qp] * _c4[_qp] * (_MVa2[_qp]/S/S) * _Zg[_qp] * _kappa_cgv[_qp] * (_grad_phi[_j][_qp]));
