...
UIVa2Rate::UIVa2Rate(const InputParameters & parameters) :
    Kernel(parameters),
...
UIVa2Rate::computeQpResidual()
{
  ...
  Real DF = _dgHVaUI[_qp] + _EBVaVa[_qp] - _kT[_qp]*(_SBVaVa[_qp])
            + _kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,0)
            - 1.0 * _kT[_qp] * MathUtils::poly4Log(1-_u[_qp],tol,0) + _kT[_qp] * MathUtils::poly4Log(_c4[_qp],tol,0)
  ...
  RealGradient DF2 = _kappa_cui[_qp] * _grad_u[_qp]
                     - 1.0 * _kappa_cui[_qp] * -_grad_u[_qp]
                     + _kappa_cvv[_qp] * _grad_c4[_qp]
                     - 2.0 * _kappa_cu[_qp] * _grad_CU[_qp]
                     - (_kappa_cv[_qp] * _grad_c2[_qp] - 1.0 * _kappa_cu[_qp] * _grad_CU[_qp]);

  Real  R1 = _test[_i][_qp] * (_c4[_qp] * _u[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp] * DF);

  Real  R2 = _test[_i][_qp] * (-_c4[_qp] * _u[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp] * DF);

  Real  R21 = _grad_test[_i][_qp] * (_c4[_qp] * _u[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp] * DF2);

  Real  R22 = _grad_test[_i][_qp] * (-_c4[_qp] * _u[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp] * DF2);

  if (_c4[_qp] >= 0 && _u[_qp] >= 0)
    return R1 + R21;
  else if (_c4[_qp] < 0.0 && _u[_qp] < 0.0)
    return R1 + R21;
  else if (_c4[_qp] < 0 || _u[_qp] < 0)
    return R2 + R22;
  else
    return R2 + R22;
...
UIVa2Rate::computeQpJacobian()
{
  ...
  Real R1 =   _test[_i][_qp] * (_phi[_j][_qp] * _c4[_qp] * (_MUI[_qp]/S/S+_MVa2[_qp]/S/S) * _Zg[_qp] * DF)
              + _test[_i][_qp] * (_u[_qp] * _c4[_qp] * (_MUI[_qp]/S/S+_MVa2[_qp]/S/S) * _Zg[_qp]
                                  * (_kT[_qp] * MathUtils::poly4Log(_u[_qp], tol, 1) * _phi[_j][_qp])
                                  - 1.0 * _kT[_qp] * MathUtils::poly4Log(1.0 - _u[_qp], tol, 1) * -_phi[_j][_qp]
                                 );

  Real R2 =  -_test[_i][_qp] * (_phi[_j][_qp] * _c4[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp] * DF)
             - _test[_i][_qp] * (_u[_qp] * _c4[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S)
                                 * _Zg[_qp] * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp])
                                 -1 * _kT[_qp] * MathUtils::poly4Log(1 - _u[_qp],tol,1) * (-_phi[_j][_qp]));

  Real R21 = _grad_test[_i][_qp] * (_c4[_qp] * _phi[_j][_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp] * DF2)
             + _grad_test[_i][_qp] * (_c4[_qp] * _u[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp]
                                      * (_kappa_cui[_qp] * (_grad_phi[_j][_qp]) - 1 * _kappa_cui[_qp] * (-_grad_phi[_j][_qp])));

  Real R22 = -_grad_test[_i][_qp] * (_c4[_qp] * _phi[_j][_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S) * _Zg[_qp] * DF2)
             - _grad_test[_i][_qp] * (_c4[_qp] * _u[_qp] * (_MUI[_qp]/S/S + _MVa2[_qp]/S/S)
                                      * _Zg[_qp] * (_kappa_cui[_qp] * (_grad_phi[_j][_qp]) - 1 * _kappa_cui[_qp] * (-_grad_phi[_j][_qp])));
