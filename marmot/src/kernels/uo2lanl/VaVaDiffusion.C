...
VaVaDiffusion::VaVaDiffusion(const InputParameters & parameters) :
    CHBulk(parameters),
...
VaVaDiffusion::computeGradDFDCons(PFFunctionType type)
{
  //VaVa diffusion
  ...
  switch (type)
  {
    case Residual:
      return _u[_qp] * (
               ...
               + _LXeVa2[_qp] * _grad_c1[_qp]
               + _LUVa2[_qp] * _grad_cu[_qp]
               - 2.0 * _LUVa2[_qp] * _grad_u[_qp]
               - 2.0 * _LXeVaU[_qp] * _grad_c3[_qp]
               + _LXeVaVa2[_qp] * _grad_c3[_qp]
               + _LVaVa2[_qp] * _grad_c2[_qp]
             ) + _kT[_qp] * (_grad_u[_qp] - 2.0 * _grad_cu[_qp] * _u[_qp] / _cu[_qp]);

    case Jacobian:
    {
      RealGradient JTERM1 = _u[_qp] * (-2.0 * _LUVa2[_qp] * _grad_phi[_j][_qp]);
      ...
      RealGradient JTERM3 = _kT[_qp] * (_grad_phi[_j][_qp] - 2.0 * _grad_cu[_qp] * _phi[_j][_qp] / _cu[_qp]);
