...
template<>
InputParameters validParams<XeVaXeVaRate>()
{
...
XeVaXeVaRate::XeVaXeVaRate(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
XeVaXeVaRate::computeQpResidual()
{
  ...
  R1 = _test[_i][_qp] * (_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S) * _Zg[_qp] * DF);

  R2 = _test[_i][_qp] * (-_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S) * _Zg[_qp] * DF);

  R21 = _grad_test[_i][_qp] * (_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S) * _Zg[_qp] * DF2);

  R22 = _grad_test[_i][_qp] * (-_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S) * _Zg[_qp] * DF2);

  // Check sign to avoid negative concentrations
  if (_c3[_qp] >= 0 && _c3[_qp] >= 0)
    return R1 + R21;
  else if (_c3[_qp] < 0.0 && _c3[_qp] < 0.0)
    return R1 + R21;
  else if (_c3[_qp] < 0 || _c3[_qp] < 0)
    return R2 + R22;
  else
    return R2 + R22;
}
...
Real
XeVaXeVaRate::computeQpJacobian()
{
  const Real tol = _LogTol[_qp];
  ...
  R1a = _test[_i][_qp] * (_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S) * _Zg[_qp] * DF);

  R21a = _grad_test[_i][_qp] * (_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S) * _Zg[_qp] * DF2);

  R1 = _test[_i][_qp] * (_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S) * _Zg[_qp]
                         * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R2 = _test[_i][_qp] * (-_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S) * _Zg[_qp]
                         * (_kT[_qp] * MathUtils::poly4Log(_u[_qp],tol,1) * _phi[_j][_qp]));

  R21 = _grad_test[_i][_qp] * (_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S)
                               * _Zg[_qp]) * (_kappa_cggv[_qp] * _grad_phi[_j][_qp]);

  R22 = _grad_test[_i][_qp] * (-_c3[_qp] * _c3[_qp] * (_MXe[_qp]/S/S)
                               * _Zg[_qp]) * (_kappa_cggv[_qp] * _grad_phi[_j][_qp]);
