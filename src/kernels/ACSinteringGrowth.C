...
Real
ACSinteringGrowth::computeDFDOP(PFFunctionType type)
{
  Real SumEtaj = 0.0;
  for (unsigned int i = 0; i < _ncrys; ++i)
  ...
  switch (type)
  {
    case Residual:
      ...
      return 12.0*_B* ((1-_rho[_qp]) * _u[_qp] - (2 - _rho[_qp])*_u[_qp]*_u[_qp] + _u[_qp]*SumEtaj) + tgrad_correction;

    case Jacobian:
      if (_has_T)
        tgrad_correction = _tgrad_corr_mult[_qp]*_grad_phi[_j][_qp]*(*_grad_T)[_qp];
      return 12.0*_B*(_phi[_j][_qp]*( - 2*(2 - _rho[_qp])*_u[_qp] + SumEtaj)) + tgrad_correction;
  }
...
Real
ACSinteringGrowth::computeQpOffDiagJacobian(unsigned int jvar)
{
  for (unsigned int i = 0; i < _ncrys; ++i)
  {
    if (jvar == _vals_var[i])
