...
InputParameters validParams<PoreGBPurdueKernelAction>()
{
  ...
  params.addRequiredParam<VariableName>("c", "Name of coupled concentration variable");
...
PoreGBPurdueKernelAction::PoreGBPurdueKernelAction(const InputParameters & params) :
    Action(params),
...
PoreGBPurdueKernelAction::act()
{
  ...
  for (unsigned int op = 0; op < _op_num; ++op)
  {
    ...
    InputParameters poly_params = _factory.getValidParams("ACPoreGBPurdue");
    poly_params.set<NonlinearVariableName>("variable") = var_name;
    poly_params.set<std::vector<VariableName> >("v") = v;
    poly_params.set<std::vector<VariableName> >("c").push_back(_c_name);
    ...
    _problem->addKernel("ACPoreGBPurdue", kernel_name, poly_params);
